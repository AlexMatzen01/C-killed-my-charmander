...
// Helper: find a pokemon by exact name (case-sensitive)
bool findPokemonByName(const string& name, Pokemon& out)
{
    auto it = find_if(pokedex.begin(), pokedex.end(), [&](const Pokemon& p) { return p.name == name; });
    if (it == pokedex.end()) return false;
    out = *it;
    return true;
}

// Generate a random wild pokemon name depending on gym level (0..n)
...
bool riddleQuestion(const string& question, const vector<string>& options, char correct)
{
    ...
    cout << "Wrong again — you suffer a minor penalty (skip next wild encounter).\n";
    return false;
}

int main()
{
    cout << "Welcome to 'pokemon but worst' (simple prototype)\n";
    ...
    // Starter selection
    cout << "\nChoose your starter: 1) Bulbasaur  2) Charmander  3) Squirtle\n:> ";
    int starterChoice = 0;
    cin >> starterChoice;
    string starterName;
    ...
    if (playerPokemon.name == "Bulbasaur") rivalPokemon = pokedex[1]; // Charmander
    else if (playerPokemon.name == "Charmander") rivalPokemon = pokedex[2]; // Squirtle
    else rivalPokemon = pokedex[0]; // Bulbasaur
    ...
    cout << "\nWhile traveling you encounter a wild " << wild.name << " (Lv " << wild.level << "). You automatically receive it as a catch reward.\n";
    ...
    for (size_t i = 0; i < gymRiddles.size(); ++i) {
        ...
        if (!riddlePassed) {
            cout << "You failed the riddle. You may retry the gym later.\n";
            continue;
        }
        cout << "Riddle passed. Prepare for the gym battle!\n";
        ...
        cout << "Choose which Pokemon to send: 1) " << playerPokemon.name << "  2) " << wild.name << "\n:> ";
        int sendChoice = 1;
        cin >> sendChoice;
        ...
        if (gymWon) {
            cout << "You defeated the gym leader's Pokemon. Gym " << (i + 1) << " cleared!\n";
            gyms_beaten++;
        }
        else {
            cout << "You lost the gym battle. Try training and come back.\n";
        }
    }

    cout << "\nGame session ended. Gyms beaten: " << gyms_beaten << "\n";
    cout << "Thanks for playing this prototype. Expand pokedex and gym mechanics next.\n";
    return 0;
}
